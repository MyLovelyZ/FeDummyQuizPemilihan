...
export default function QuizCreate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/quizzes/${res.data.data.id}`)
    } catch (err) {
      const errs = err.response?.data?.errors
      const msg = errs ? Object.values(errs).flat().join(' ') : (err.response?.data?.message || 'Gagal membuat quiz.')
      setError(msg)
    } finally {
      setSaving(false)
    }
    ...
    <Layout>
      <div className="max-w-xl space-y-4">
        <div className="flex items-center gap-3">
          <Link to="/quizzes" className="text-sm text-gray-500 hover:text-gray-700">← Kembali</Link>
